  ...
  return (
    <>
      <BaseRadio.Group value={selectedOption} onChange={(e) => setSelectedOption(e.target.value)}>
        <S.RadioBtn value="phone" $isActive={isPhoneActive} disabled={user?.phone ? false : true}>
          <PhoneItem required={isPhoneActive} onClick={onClickInput('phone')} verified={user?.phone ? false : true} />
        </S.RadioBtn>
        <S.RadioBtn value="email" $isActive={isEmailActive} disabled={user?.email ? false : true}>
          <EmailItem required={isEmailActive} onClick={onClickInput('email')} verified={user?.email ? false : true} />
        </S.RadioBtn>
      </BaseRadio.Group>
    </>
  );
